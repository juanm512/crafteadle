...
function App() {
  ...
  const [ dayRecipe, setDayRecipe ] = React.useState(null);
  ...
  const [ background, setBackground ] = React.useState( localStorage.getItem('background') || 'performance' );
  ...
  React.useEffect(() => {
    ...
    .then( async data => {
      ...
      if( localGameResume && localGameResume.dayRecipeName === data.result.item ) {
        ...
        setGameResume({
          ...
          gameWon: false,
          heartsAmount: 8,
          dayRecipeName: data.result.item, 
          craftingsTrys: []
        });
      }
    }).catch(err => {
    ...
    .then(data => {
      ...
      for( let i = 0; i < 27 - data.length; i++ ) {
        parsedData.push({
          id: 'empty' + (Math.random() * 100).toString() ,
          name: 'empty',
          quantity: 0,
  ...
  const handleCraftingButton = () => { //function to handle the crafting button
    ...
    if ( craftingTable.every( item => item.name === "empty" ) ){
      ...
      //check if the crafting table has the correct pattern of items to craft the recipe
      let newCraftingTable = [...craftingTable];
      ...
      if( craftingTablePattern.every( (row,i) => row.every( (item,j) => item === parsedDayPattern[i][j] ) ) ){
          ...
          <button
            ...
            { background === 'performance' ? 
              <div className="bg-black h-screen w-screen">
                <img src={require('./images/Craftdle@1-1366x643.png')} alt="CraftdleBG" className="h-screen w-screen object-cover" />
              </div>
            :
              <Spline
                style={{
        ...
        { (menuUI === 'crafteadle' && !gameResume.gameOver ) &&
          <>
            ...
            <div className="main2">
              ...
              { background === 'performance' ? 
              <div className="bg-black h-screen w-screen">
                <img src={require('./images/Untitled@1-1366x643.png')} alt="CraftdleBG" className="h-screen w-screen object-cover" />
              </div>
              :
              <Spline style={{
                  width: '100%',
